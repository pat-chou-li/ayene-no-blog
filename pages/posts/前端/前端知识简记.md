...
- 数据如何影响视图：响应式原理
- 视图如何影响数据：监听dom时间，addEventListener
...
- 引入了**头信息压缩**，一方面，头信息使用 Head compress（Hpack算法，包含静态表、动态表和huffman编码） 压缩后再发送；另一方面， 客户端和服务器同时维护一张头信息表，所有字段都会存入这个表，生成一个索引号，以后就不发送同样字段了，只发送索引 号，这样就能提高速度了。
- 资源主动推送
